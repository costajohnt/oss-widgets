...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const cacheKey = `top-repos:${username}:${mode}:${minStars}`;
  ...
  // Fetch contribution data + star counts, race against timeout
  const computation = (async () => {
    const result = await fetchContributionData(username, process.env.GITHUB_TOKEN!);
    if (result.error) return { error: result.error };

    // Fetch star counts for the top repos to enable minStars filtering
    const repoNames = result.topRepos.map((r) => r.repo);
    const octokit = new Octokit({ auth: process.env.GITHUB_TOKEN });
    const starsMap = await getStarsForRepos(octokit, repoNames);

    // Filter by minStars and attach star counts
    const filtered = result.topRepos
      .filter((r) => (starsMap.get(r.repo) ?? 0) >= minStars)
      .slice(0, 8);

    return { ...result, topRepos: filtered };
  })();

  computation.catch((err) => {
    console.error(`[top-repos] Post-timeout error for ${username}:`, err instanceof Error ? err.message : String(err));
  });
  ...
  if (outcome === 'timeout' || (typeof outcome === 'object' && 'error' in outcome && outcome.error)) {
    ...
    if (stale && Date.now() - stale.ts < STALE_TTL) {
      ...
      console.warn(`[top-repos] Serving stale cache for ${username}: ${reason}`);
  ...
  const svg = renderTopReposCard(outcome, mode);
